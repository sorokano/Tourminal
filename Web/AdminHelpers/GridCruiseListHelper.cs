...
namespace Elcondor.AdminHelpers {
    public static class GridCruiseListHelper {
        public static string GetGridHTML (int filterCountryId, int sortByField, bool isDesc) {
            ...
            List<TblTour> list = (filterCountryId > 0)
                ? BizTour.GetCruiseListByCountryId(filterCountryId)
                : BizTour.GetCruiseListByCountryId(null);
        ...
        private static List<TblTour> ApplySorting (List<TblTour> list, int sortByField, bool isDesc) {
            var sortedCruises = from p in list
                              select p;

            switch (sortByField) {
                case Constants.ColTourListId:
                    sortedCruises = (isDesc) ? from p in list orderby p.Id descending select p
                                                : from p in list orderby p.Id ascending select p;
                    break;
                case Constants.ColTourListName:
                    sortedCruises = (isDesc) ? from p in list orderby p.Name descending select p :
                                        from p in list orderby p.Name ascending select p;
                    break;
                //case Constants.ColTourListCountry:
                //    sortedCruises = (isDesc) ? from p in list orderby p.RegionId descending select p :
                //                        from p in list orderby p.RegionId ascending select p;
                //    break;
                default:
                    sortedCruises = from p in list orderby p.Id ascending select p;
                    break;
            }
            return sortedCruises.ToList();
        }
        ...
        private static string GetFormattedRow (string id, string name, string countryReg) {
            ...
            stringBuilder.Append("<td>");
            stringBuilder.Append(string.Format("<a href=\"/Admin/CruiseEdit?id={0}\">{0}</a>", id));
            stringBuilder.Append("</td>");
            stringBuilder.Append("<td>");
            stringBuilder.Append(string.Format("<a href=\"/Admin/CruiseEdit?id={0}\">{1}</a>", id, name));
            stringBuilder.Append("</td>");
            stringBuilder.Append("<td>");
