    ...
    public static class GridTourListHelper {
        public static string GetGridHTML (int filterCountryId, int sortByField, bool isDesc) {
            ...
            List<TblTour> list = (filterCountryId > 0)
                ? BizTour.GetTourListByCountryId(filterCountryId)
                : BizTour.GetTourListByCountryId(null);
        ...
        private static List<TblTour> ApplySorting (List<TblTour> list, int sortByField, bool isDesc) {
            var sortedTours = from p in list
                                     select p;

            switch (sortByField) {
                case Constants.ColTourListId:
                    sortedTours = (isDesc) ? from p in list orderby p.Id descending select p
                                                : from p in list orderby p.Id ascending select p;
                    break;
                case Constants.ColTourListName:
                    sortedTours = (isDesc) ? from p in list orderby p.Name descending select p :
                                        from p in list orderby p.Name ascending select p;
                    break;
                //case Constants.ColTourListCountry:
                //    sortedTours = (isDesc) ? from p in list orderby p.RegionId descending select p :
                //                        from p in list orderby p.RegionId ascending select p;
                //    break;
                default:
                    sortedTours = from p in list orderby p.Id ascending select p;
                    break;
            }
            return sortedTours.ToList();
        }
        ...
        private static string GetHeader (int filterCountry) {
            ...
            stringBuilder.Append("Страна, регион");
            stringBuilder.Append("        </td>");
        ...
        private static string GetFormattedRow (string id, string name, string countryReg) {
            ...
            stringBuilder.Append("<td>");
            stringBuilder.Append(string.Format("<a href=\"/Admin/TourEdit?id={0}\">{0}</a>", id));
            stringBuilder.Append("</td>");
            stringBuilder.Append("<td>");
            stringBuilder.Append(string.Format("<a href=\"/Admin/TourEdit?id={0}\">{1}</a>", id, name));
            stringBuilder.Append("</td>");
            stringBuilder.Append("<td>");
